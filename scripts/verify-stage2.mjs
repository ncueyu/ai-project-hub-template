...
const CONTENT_CONTRACTS = [
  ...
  /*
   * 密碼與權限的時序（2026-08-29 建立；2026-09-05 整組反轉）。
   *
   * 這兩條原本釘的是相反的話——「改完密碼要重新部署那個專案才會生效」。
   * 當時密碼確實是部署當下烙進去的，那句提醒是刻意的防呆，所以用釘樁守著。
   *
   * 2026-09-04 的 commit 8bf7931「權限改成即時生效」把行為改掉了，也正確地
   * 從後台與 AGENTS.md 刪掉那句話——但**沒有一起改這支腳本**，於是這兩條
   * 從那天起一直在要求兩個檔案講一件已經不成立的事，`npm run verify` 也就
   * 一直是紅的。更糟的是它的失敗訊息會引導人「把那句話補回去」，等於把錯誤
   * 的指示放回教材裡。
   *
   * 現在改成釘住新的事實。守的東西沒變——都是「使用者對時序的預期」這一類
   * 最容易造成危險誤解的說明：以前的風險是「以為鎖好了其實還沒生效」，
   * 現在的風險是「被叫去做一次根本不需要的重新部署」，兩者都會讓人下一次
   * 不再相信我們講的時序。
   *
   ...
   */
  {
    ...
    must: ["改完按「儲存密碼」就生效"],
    label: "後台有說明：改完密碼立即生效，不必重新部署",
  },
  {
    file: "AGENTS.md",
    must: ["權限與密碼都是即時生效的，不要叫使用者重新部署"],
    label: "AGENTS.md 要求 AI 不要叫使用者為了權限或密碼重新部署",
  },
];
