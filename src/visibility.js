 ...
 * 也不需要任何驗證，因此**不應該**注入閘道。多注入一層不只浪費
 * Worker 額度，還會讓原本免費且無上限的靜態請求變成計費請求。
 *
...
/**
 * 產生該專案部署時應使用的 `run_worker_first` 設定。
 *
 * 不需要閘道的狀態一律回傳空陣列，代表靜態資源直接送出、不經過 Worker：
 * 這樣請求免費且無上限。需要閘道時才涵蓋全部路徑，確保連子資源
 * （CSS、JS、圖片）都無法繞過檢查。
 *
 * @param {string} visibility
 * @returns {string[]}
 */
export function runWorkerFirstFor(visibility) {
